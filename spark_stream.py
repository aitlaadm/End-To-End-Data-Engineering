...
def create_table(session):
    session.execute("""
                    CREATE TABLE IF NOT EXISTS spark_streams.created.created_users(
                        id UUID PRIMARY KEY,
                        first_name TEXT,
                        last_name TEXT,
                        gender TEXT,
...
def insert_data(session, **kwargs):
    ...
    first_name=kwargs.get('firs_tname')
...
#conneting to the cassandra cluster
    cass_session=None
    try:
...
if __name__=="__main__":
    ...
    if spark_conn is not None:
        ...
        if session is not None:
            ...
            # insert_data(session)
            streaming_query= (selection_df.writeStream.format("org.apache.spark.sql.cassandra").option('checkpointLocation', '/tmp/checkpoint').option('keyspace','spark_streams')  .option('table','created_users').start())
